...
import DeleteBin from '../../icons/majesticons--delete-bin.svg'
import '../../App.css'
...
export default function Selection(state: GroupingsUIState)  {
  ...
  table.dataTransform = {
    Name: (value) => {
      ...
      <div style=" display: flex; justify-content: space-between; flex: 1; align-items: center;">
        ...
        <button onmouseover="this.style.backgroundColor = 'var(--bim-ui_accent-base)'"
                onmouseout="this.style.backgroundColor = 'var(--bim-ui_gray-10)'"
                @click=${onDeleteGroup} 
                style="  
                  width: 28px ;
                  height: 28px;
                  ...
                  border: 0px;
                  border-radius: 15%;
                  background-color: var(--bim-ui_gray-10);
                  transition: 0.15s">
            <img style=" width: 14px;" src=${DeleteBin} alt="DeleteBin"/>
        </button>
      </div> 
      `;
    },
